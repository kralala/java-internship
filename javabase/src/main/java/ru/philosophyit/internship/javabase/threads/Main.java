...
public class Main {
    ...
    // Потому что метод shutdown() запрещает отправлять новые таски, но не прекращает выполнение уже существующих
    ...
    // Потому что у потоков не изменился статус, несмотря на закрытие всех задач
    ...
    // Потому что имеем только 4 потока (строка 22)
